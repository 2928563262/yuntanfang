...
### 2026-06-06 功能说明书页面缺口盘点

- 已根据 `云摊坊_功能数据设计说明书.md` 对比当前 H5、商家端和管理后台路由。
- 新增 `docs/frontend-page-gap-analysis.md`，记录当前已覆盖页面、仍是骨架的页面、说明书要求但缺失的页面和推荐开发顺序。
- “我的”页退出登录卡片去掉下方说明小字。

### 2026-06-06 管理端 UI 克制优化

- 使用 `frontend-design` skill，但按后台工具场景控制视觉强度，不做花哨装饰。
- 管理端改为灰白底、深绿主色、低阴影、紧凑卡片和清晰表格层级。
...
### 2026-06-07 用户端闭环与 Agent 接入

- 后端新增 `/api/agent/order/parse`，通过 `DEEPSEEK_*` 环境变量接 DeepSeek OpenAI-compatible 接口。
- 未配置 DeepSeek key 时，Agent 接口自动返回本地降级解析，避免前端流程中断。
- H5 智能点单页接入真实后端代理，发送用户输入后生成消息回复和订单确认卡。
- 摊位详情补顾客评价列表和摊主故事入口。
- 预约下单页补商品、数量、取货时间、联系方式与订单确认摘要，提交后进入订单详情。
- 订单列表和评价提交补闭环入口。

### 2026-06-07 用户端核心闭环强化

- H5 新增 `src/stores/userData.ts`，用 localStorage 临时持久化用户订单、投诉、收藏、提醒和评价。
- 预约下单和智能点单确认卡均可创建本地订单，并跳转到对应订单详情。
- 订单详情支持待支付、支付成功、确认取餐、取消订单、退款中等状态流转，并展示取餐码。
- 评价提交写入“我的评价”，同时同步回摊位详情评价列表。
- 投诉提交写入“我的投诉”，收藏页支持取消收藏，摊位详情支持收藏/取消收藏和订阅/取消提醒。
